...
import kotlinx.android.synthetic.main.fragment_comp_home_confirmed.view.*
import kotlinx.android.synthetic.main.fragment_comp_home_confirmed.view.linNoMore
import kotlinx.android.synthetic.main.fragment_comp_home_confirmed.view.loadView
import kotlinx.android.synthetic.main.fragment_comp_home_confirmed.view.txtDateRange
...
class CompHomeConfirmedFragment(
    ...
    lateinit var confirmedViewAdapter: HomeConfirmedViewAdapter
    ...
    override fun onResume() {
        ...
        //call api for confirmed reservation listing
        resirvationListApi("", Utility.RES_CONFIRMED_STATUS,activityBase)
    ...
    override fun onCreateView(
        ...
        rootView = inflater.inflate(R.layout.fragment_comp_home_confirmed, container, false)
        handler = Handler()
        clickView()
    ...
    private fun clickView() {
        rootView.recyclerConfirmed.setOnScrollChangeListener { view, i, i2, i3, i4 ->
    ...
    private fun resirvationListApi(type: String, status: String, requireActivity: Activity) {
        ...
        CallApi.callAPi(mBuilder, API, requireActivity, Utility.reservationsConfirmed, pageCount==1, Utility.GET, true)
    }


    override fun onTaskComplete(result: JSONObject, type: String, isData: Boolean) {
        if (type==Utility.reservationsConfirmed){
            if (isData){
                ...
                confirmedCount=data.getInt(Utility.key.total)
                try {
                    companyHomeFragment.tab_layout.getTabAt(1)!!.text = companyHomeFragment.tabsArray[1]+" (${confirmedCount})"
                ...
                if (total<=0){
                    ...
                    rootView.recyclerConfirmed.showOrGone(false)
                }else{
                    rootView.linNoMore.showOrGone(false)
                    rootView.recyclerConfirmed.showOrGone(true)
                    ...
                    if (pageCount == 1) {
//                        rootView.recyclerConfirmed.setHasFixedSize(true)
                        linearLayoutManager = LinearLayoutManager(
                            activityBase,
                            LinearLayoutManager.VERTICAL,
                            false
                        )
                        confirmedViewAdapter =
                            HomeConfirmedViewAdapter(activityBase, resverationItems)
                        rootView.recyclerConfirmed.layoutManager = linearLayoutManager
                        rootView.recyclerConfirmed.adapter = confirmedViewAdapter
                    }else{
                        rootView.loadView.showOrGone(false)
                        confirmedViewAdapter.notifyDataSetChanged()
                    }
                }
