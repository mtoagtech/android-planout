...
//        holder.recyclerTags.setHasFixedSize(true)
        ...
        holder.itemView.setOnClickListener {
            resultLauncherFavoriteInd.launch(Intent(activity, BusinessDetailsActivity::class.java)
...
//                .putExtra(Utility.key.id,item.id))
            ...
            if (Utility.isLoginCheck(activity)) {
                if (Utility.hasConnection(activity)) {
                    if (item.is_favorite == "true") {
                        ...
                        HomeVisitorActivity.myObject(activity, item.id, item.store_name, "true")
                        VisitorSearchActivity.setSearchFavView(item.id, item.store_name, "true")
                        removeFavApi(item.id)
                    } else {
                        item.is_favorite = "true"
                        holder.imgFav.setImageResource(R.drawable.ic_icon_fav_true)
                        HomeVisitorActivity.myObject(activity, item.id, item.store_name, "false")
                        VisitorSearchActivity.setSearchFavView(item.id, item.store_name, "false")
                        addFavApi(item.id)
                    }
                }
            }
    ...
    private fun addFavApi(id: String) {
        ...
        CallApi.callAPi(mBuilder, "${ApiController.api.favorites}/$id", activity!!, Utility.favoritesAdd, true, Utility.POST, true)

    }

    private fun removeFavApi(id: String) {
        val mBuilder = FormBody.Builder()
        CallApi.callAPi(mBuilder, "${ApiController.api.favorites}/$id", activity!!, Utility.favoritesRemove, true, Utility.DELETE, true)
