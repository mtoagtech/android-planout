...
class HomeConfirmedViewAdapter(
    val activity: Activity?,
    val resverationItems: ArrayList<ReservationData>
) : RecyclerView.Adapter<HomeConfirmedViewAdapter.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.row_company_confirmed_list_view, parent, false)
    ...
    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        ...
        if(item.is_arrived == 1){
            holder.switchArived.setToggleOn()
            holder.arivedText.text = "Arrived"
        }else{
            holder.arivedText.text = "Not Arrived"

            holder.switchArived.setToggleOff()
        }
        holder.switchArived.setOnToggleChanged {
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val arivedText = itemView.findViewById<TextView>(R.id.arivedText)
        val view5 = itemView.findViewById<View>(R.id.view5)
        val switchArived = itemView.findViewById<ToggleButton>(R.id.switchArived)
    }
}